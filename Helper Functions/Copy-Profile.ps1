    ...
    .NOTES
        Author: Dom Ruggeri
        Last Edit: 1/15/2024 to add the ability to use an existing target folder. 
        Fixed the Robocopy arguments to exclude ntuser.* files if using existing target.
        Added back the recycle bin to copy instead of being excluded. 
    ...
    Begin {
        function Copy-WithProgress {
            ...
            If ($UseExistingTarget){
                $CommonRobocopyParams = '/COPYALL /E /XO /XF ntuser.* /NP /NDL /NC /BYTES /NJH /NJS /XJ /TEE /R:0'
            }
            ELSEIF ($IncludeRobocopyDetail) {
            ...
            $CommonRobocopyExcludes = "/XD ""$ProfilePath`System Volume Information"""
